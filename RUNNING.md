...
**Note:** For higher hash bit sizes (`-hb`), query generation can take a while.  If extra cores are available, this step can be sped up by increasing the number of threads (e.g. `-nt 4`).  In addition, the "fast" and "fastwithjni" methods should be significantly faster than the default method.  To use the "fastwithjni" method, you will need to compile a native library separately (located in the `jni/querier/` source subdirectory).  This should produce a library file called `libquerygen.so` or `libquerygen.dylib` (depending on your platform), which you can optionally copy to a more convenient location.  When creating the query file, the `java.library.path` property can be used to indicate where to locate the library:
```

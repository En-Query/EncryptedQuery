 ...
 * the License.
 */
package org.enquery.encryptedquery.pig.mini;
...
/**
 * This class builds a single instance of itself with the Singleton design pattern. While building
 * the single instance, it sets up a mini cluster that actually consists of a mini DFS cluster and a
 * mini MapReduce cluster on the local machine and also sets up the environment for Pig to run on
 * top of the mini cluster.
 *
 * This class is the base class for MiniCluster, which has slightly difference among different
 * versions of hadoop. MiniCluster implementation is located in $PIG_HOME/shims.
 */
abstract public class MiniGenericCluster {
	...
	/**
	 * Returns the single instance of class MiniGenericCluster that represents the resources for a
	 * mini dfs cluster and a mini mr (or tez) cluster. The system property "test.exec.type" is used
	 * to decide whether a mr or tez mini cluster will be returned.
	 */

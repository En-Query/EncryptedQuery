...
int main()
{
	...
	cout << " 10. My Test Program" << endl;
	cout << " 11. Relinearization vs multiplication" << endl;
	cout << " 12. Four Multiplications comparison" << endl;
	cout << " 13. Full Test Program" << endl;
	cout << " 14. Differenet Monomials" << endl;
	cout << " 15. Test Variables" << endl;
	cout << " 16. Sum Noise Growth" << endl;
        cout << " 0. Exit" << endl;
...
void sumnoise()
{
    print_example_banner("Different Monomials");
    ...
    Plaintext plain0("0");
    Ciphertext ciphers[1000];
    Ciphertext accum;
    fstream fout;
    fout.open("SumNoise.txt",fstream::out);
    for(int i=0;i<1000;i++)
    {
	    encryptor.encrypt(plain0,ciphers[i]);
    }
    accum=ciphers[0];
    fout << decryptor.invariant_noise_budget(accum) <<endl;
    for(int i=0;i<1000;i++)
    {
	evaluator.add_inplace(accum,ciphers[i]);
        fout << decryptor.invariant_noise_budget(accum) <<endl;
    }
    fout.close();
}
void testvariables()
{
    Plaintext plain[LISTLEN][LISTLEN][LISTLEN][LISTLEN];
    Plaintext plain0("0");
    Plaintext plain1("1");
    Ciphertext list1[LISTLEN];
    Ciphertext list2[LISTLEN];
    Ciphertext list3[LISTLEN];
    Ciphertext list4[LISTLEN];
    Ciphertext list12[LISTLEN][LISTLEN];
    Ciphertext list34[LISTLEN][LISTLEN];
    Ciphertext accum;
    Ciphertext list1234[LISTLEN][LISTLEN][LISTLEN][LISTLEN];
    uint64_t r;
    auto t1=Clock::now();
    auto t2=Clock::now();
    auto duration=timediff(t1,t2);
 
    fstream fout;
    fout.open("TestVar.txt",fstream::out);
    char buffer[8];
    for(int t=2;t<33;t=t<<1)
    for(int d=1024;d<17000;d=d<<1)
    {
    uint64_t t_exp =UINT64_C(1)<<t;
    EncryptionParameters parms(scheme_type::BFV);
    if(t>16&&d==1024)
	    d=2048;
    parms.set_poly_modulus_degree(d);
    parms.set_coeff_modulus(DefaultParams::coeff_modulus_128(d));
    parms.set_plain_modulus(t_exp);
    auto context = SEALContext::Create(parms);
    print_parameters(context);
    KeyGenerator keygen(context);
    auto public_key = keygen.public_key();
    ...
    Evaluator evaluator(context);
    Decryptor decryptor(context, secret_key);
    uint64_t t_m1=t_exp-1;
    //generate database as random table in our domain
    ...
    for(int l=0;l<LISTLEN;l++)
    {
        ...
        while(r==0)
            r=rand()&t_m1;
        sprintf(buffer,"%x",r);
        plain[i][j][k][l]=Plaintext(buffer);
    }
    duration=0;
    //generate Ciphertext encryption lists
    for(int i=0;i<LISTLEN;i++)
    {
	t1=Clock::now();
        if(i==1)
	    encryptor.encrypt(plain1,list1[i]);
	else
	    encryptor.encrypt(plain0,list1[i]);
	if(i==2)
	    encryptor.encrypt(plain1,list2[i]);
	else
	    encryptor.encrypt(plain0,list2[i]);
	if(i==1)
	    encryptor.encrypt(plain1,list3[i]);
	else
	    encryptor.encrypt(plain0,list3[i]);
	if(i==3)
	    encryptor.encrypt(plain1,list4[i]);
	else
	    encryptor.encrypt(plain0,list4[i]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
	
    }
    fout << "DE " << t << " " << d << " 0 " << duration << endl;
    fout << "DE " << t << " " << d << " 16 " << duration << endl;
    fout << "DE " << t << " " << d << " 32 " << duration << endl;
    fout << "DE " << t << " " << d << " 48 " << duration << endl;
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    {
        t1=Clock::now();
	evaluator.multiply(list1[i],list2[j],list12[i][j]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
    }
    fout << "D12 " << t << " " << d << " 0 " << duration << endl;
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    {
        t1=Clock::now();
	evaluator.multiply(list3[i],list4[j],list34[i][j]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
    }
    fout << "D34 " << t << " " << d << " 0 " << duration << endl;
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    for(int k=0;k<LISTLEN;k++)
    ...
    {
        t1=Clock::now();
	evaluator.multiply(list12[i][j],list34[k][l],list1234[i][j][k][l]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
 
    } 
    fout << "D1234 " << t << " " << d << " 0 " << duration << endl;
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    for(int k=0;k<LISTLEN;k++)
    ...
    {
        t1=Clock::now();
	evaluator.multiply_plain_inplace(list1234[i][j][k][l],plain[i][j][k][l]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
 
    } 
    fout << "DP " << t << " " << d << " 0 " << duration << endl;
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    for(int k=0;k<LISTLEN;k++)
    ...
    {
        t1=Clock::now();
	evaluator.add_inplace(accum,list1234[i][j][k][l]);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
 
    } 
    else
        accum=list1234[0][0][0][0];
    }
    fout << "DS " << t << " " << d << " 0 " << duration << endl;
    fout << "CS " << t << " " << d << " 0 " << 
	    5*d*context->context_data()->total_coeff_modulus_bit_count() << endl; 
    fout << "NB " << t << " " << d << " 0 " <<
       decryptor.invariant_noise_budget(accum) <<endl;
    for(int DBC=16;DBC<49;DBC+=16)
    {
    ...
    for(int j=0;j<LISTLEN;j++)
    {
        t1=Clock::now();
	evaluator.multiply(list1[i],list2[j],list12[i][j]);
	evaluator.relinearize_inplace(list12[i][j],relin_keys);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
    }
    fout << "D12 " << t << " " << d << " " << DBC << " " << duration << endl; 
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    {
        t1=Clock::now();
	evaluator.multiply(list3[i],list4[j],list34[i][j]);
	evaluator.relinearize_inplace(list34[i][j],relin_keys);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
    }
    fout << "D34 " << t << " " << d << " " << DBC << " " << duration << endl; 
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    for(int k=0;k<LISTLEN;k++)
    for(int l=0;l<LISTLEN;l++)
    {
        t1=Clock::now();
	evaluator.multiply(list12[i][j],list34[k][l],list1234[i][j][k][l]);
	evaluator.relinearize_inplace(list1234[i][j][k][l],relin_keys);
    	t2=Clock::now();
    	duration+=timediff(t1,t2);
    }
    fout << "D1234 " << t << " " << d << " " << DBC << " " << duration << endl; 
    duration=0;
    for(int i=0;i<LISTLEN;i++)
    for(int j=0;j<LISTLEN;j++)
    for(int k=0;k<LISTLEN;k++)
    ...
    fout << "DS " << t << " " << d << " " << DBC << " " << duration << endl;
    fout << "CS " << t << " " << d << " " << DBC << " " << 
	    2*d*context->context_data()->total_coeff_modulus_bit_count() << endl; 
    fout << "NB " << t << " " << d << " " << DBC << " " <<
       decryptor.invariant_noise_budget(accum) <<endl;
...
void diffmonomials()
{
    ...
    parms.set_plain_modulus(1 << 8);
    fstream fout;
    fout.open("DiffMon.txt",fstream::out);
    auto context = SEALContext::Create(parms);
    print_parameters(context);
    KeyGenerator keygen(context);
    auto public_key = keygen.public_key();
    ...
    for(int k=0;k<LISTLEN;k++)
	    for(int l=0;l<LISTLEN;l++)
	    {
		    ...
		    if(r==0)
			r++;
		    plain[i][j][k][l]=Plaintext(to_string(r));
	    }
    for(int n=0;n<4096;n++)
    {
	plainmono=Plaintext("1x^"+to_string(n));
   	for(int i=0;i<LISTLEN;i++)
    {
	    if(i==1)
		encryptor.encrypt(plain1,list1[i]);
		...
		encryptor.encrypt(plain0,list2[i]);
	    if(i==1)
		encryptor.encrypt(plainmono,list3[i]);
	    else
		encryptor.encrypt(plain0,list3[i]);
	    if(i==3)
		encryptor.encrypt(plain1,list4[i]);
    	...
    	for(int i=0;i<LISTLEN;i++)
    {
	    ...
	    {
	       fout << "NBB12 " << n << " " << 
		       decryptor.invariant_noise_budget(list1[i]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list1[i],list2[j],list12[i][j]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "D12 " << n << " " << duration << endl;
	       fout << "NBA12 " << n << " " <<
		       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
	       t1=Clock::now();
    	       evaluator.relinearize_inplace(list12[i][j], relin_keys);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "DR12 " << n << " " << duration <<endl;
	       fout << "NBAR12 " << n << " " <<
		       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
       	       fout << "NBB34 " << n << " " <<
		       decryptor.invariant_noise_budget(list1[i]) <<endl;
	       ...
	       evaluator.multiply(list3[i],list4[j],list34[i][j]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "D34 " << n << " " << duration << endl;
	       fout << "NBA34 " << n << " " <<
		       decryptor.invariant_noise_budget(list34[i][j]) <<endl;
	       t1=Clock::now();
    	       evaluator.relinearize_inplace(list34[i][j], relin_keys);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "DR34 " << n << " " << duration <<endl;
       	       fout << "NBAR34 " << n << " " <<
		       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
	       ...
	       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list12[i][j],list34[k][l],tmp);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "D1234 " << n << " " << duration << endl;
	       fout << "NBA1234 " << n << " " <<
	       decryptor.invariant_noise_budget(tmp) <<endl;
	       t1=Clock::now();
	       evaluator.relinearize_inplace(tmp, relin_keys);
               t2=Clock::now();
               duration=timediff(t1,t2);
               fout << "DR1234 " << n << " " << duration <<endl;
               fout << "NBAR1234 " << n << " " <<
                   decryptor.invariant_noise_budget(tmp) <<endl;
   
	       t1=Clock::now();
	       evaluator.multiply_plain_inplace(tmp,plain[i][j][k][l]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "DP " << n << " " << duration << endl;
	       fout << "NBAP " << n << " " <<
	       decryptor.invariant_noise_budget(tmp) <<endl;
               t1=Clock::now();
	       evaluator.add_inplace(accum,tmp);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "DA " << n << " " << duration << endl;
	       fout << "NBAA " << n << " " <<
	       decryptor.invariant_noise_budget(accum) <<endl;
	       ...
	       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list12[i][j],list34[k][l],accum);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "D1234 " << n << " " << duration << endl;
	       fout << "NBA1234 " << n << " " <<
               decryptor.invariant_noise_budget(accum) <<endl;
	       t1=Clock::now();
	       evaluator.relinearize_inplace(accum, relin_keys);
               t2=Clock::now();
               duration=timediff(t1,t2);
               fout << "DR1234" << " 0 " << duration <<endl;
               fout << "NBA1234" << " 0 " <<
                   decryptor.invariant_noise_budget(accum) <<endl;
   

               t1=Clock::now();
	       evaluator.multiply_plain_inplace(accum,plain[i][j][k][l]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "DP " << n << " " << duration << endl;
	       fout << "NBAP " << n << " " <<
    ...
    {
	    t1=Clock::now();
	    evaluator.add_inplace(finalaccum,accum);
    	    t2=Clock::now();
    	    duration=timediff(t1,t2);
    	    fout << "DAA " << n << " " << duration << endl;
	    fout << "NBAAA " << n << " " <<
	       decryptor.invariant_noise_budget(finalaccum) <<endl;
    ...
    t1=Clock::now();
    evaluator.relinearize_inplace(finalaccum, relin_keys);
    t2=Clock::now();
    duration=timediff(t1,t2);
    fout << "DRF" << " 0 " << duration <<endl;
    fout << "NBARF" << " 0 " <<
       decryptor.invariant_noise_budget(finalaccum) <<endl;
...
void fulltest()
{
    ...
    f1out.open("FullTestNR.txt",fstream::out);
    f2out.open("FullTestR.txt",fstream::out);
    auto context = SEALContext::Create(parms);
    print_parameters(context);
    KeyGenerator keygen(context);
    auto public_key = keygen.public_key();
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
		    for(int k;k<LISTLEN;k++)
		    {
			    for(int l;l<LISTLEN;l++)
			    if(i||j||k||l)
			    {
	       f1out << "NBB1234 " << 
	       decryptor.invariant_noise_budget(list12[i][j]) <<endl;
...
void fourmultcomp()
{
    print_example_banner("Four Multiplications comparison");
    EncryptionParameters parms(scheme_type::BFV);
    parms.set_poly_modulus_degree(4096);
    parms.set_coeff_modulus(DefaultParams::coeff_modulus_128(4096));
    parms.set_plain_modulus(1 << 8);

    fstream f1out,f2out,f3out,f4out;
    f1out.open("fourmultnestnrl.txt",fstream::out);
    f2out.open("fourmultnestrl.txt",fstream::out);
    f3out.open("fourmultnnestnrl.txt",fstream::out);
    f4out.open("fourmultnnestrl.txt",fstream::out);
    auto context = SEALContext::Create(parms);
    print_parameters(context);
    KeyGenerator keygen(context);
    auto public_key = keygen.public_key();
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
	       ...
	       evaluator.multiply(list12[i][j],list34[k][l],list1234[i][j][k][l]);
    	       ...
    	       f1out << "D1234 " << duration << endl;
	       f1out << "NBA1234 " << 
	       decryptor.invariant_noise_budget(list1234[i][j][k][l]) <<endl;
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
	       ...
	       decryptor.invariant_noise_budget(list1[i]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list1[i],list2[j],tmp);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f3out << "D12 " << duration << endl;
	       f3out << "NBA12 " << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
	       t1=Clock::now();
	       evaluator.multiply_inplace(tmp,list3[k]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f3out << "D123 " << duration << endl;
	       f3out << "NBA123 " << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
               t1=Clock::now();
	       evaluator.multiply_inplace(tmp,list4[l]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f3out << "D1234 " << duration << endl;
	       f3out << "NBA1234 " << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
...
void relinvsmult()
{
    ...
    print_example_banner("Relinearization vs. multiplication");
    EncryptionParameters parms(scheme_type::BFV);
    parms.set_poly_modulus_degree(4096);
    parms.set_coeff_modulus(DefaultParams::coeff_modulus_128(4096));
    parms.set_plain_modulus(1 << 8);
    ...
    fstream f1out,f2out;
    f1out.open("relinvsmult1.txt",fstream::out);
    f2out.open("relinvsmult2.txt",fstream::out);
    auto context = SEALContext::Create(parms);
    print_parameters(context);
    KeyGenerator keygen(context);
    auto public_key = keygen.public_key();
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    ...
	    {
		for(int k=0;k<LISTLEN;k++){
	       f1out << "Noise Budget before mult:" << 
		       decryptor.invariant_noise_budget(list1[i]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list1[i],list2[j],tmp);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f1out << "Cyphertext mult took: " << duration << "ns" <<endl;
	       f1out << "Noise Budget after mult:" << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
               t1=Clock::now();
	       evaluator.multiply_inplace(tmp,list3[k]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f1out << "Second Cyphertext mult took: " << duration << "ns" <<endl;
	       f1out << "Noise Budget after second mult:" << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
	    }
	    }
    }
    for(int i=0;i<LISTLEN;i++)
    {
	    ...
	    {
		for(int k=0;k<LISTLEN;k++){
	       f2out << "Noise Budget before mult:" << 
		       decryptor.invariant_noise_budget(list1[i]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list1[i],list2[j],tmp);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f2out << "Cyphertext mult took: " << duration << "ns" <<endl;
	       f2out << "Noise Budget after mult:" << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
               t1=Clock::now();
    	       evaluator.relinearize_inplace(tmp, relin_keys);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f2out << "Relinearization took: " << duration << "ns" <<endl;
               t1=Clock::now();
	       evaluator.multiply_inplace(tmp,list3[k]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       f2out << "Relin Cyphertext mult took: " << duration << "ns" <<endl;
	       f2out << "Noise Budget after second mult:" << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
		}
	    }
    }
...
void test_function()
{
    print_example_banner("My Test Program");
    EncryptionParameters parms(scheme_type::BFV);
    ...
    Evaluator evaluator(context);
    Decryptor decryptor(context, secret_key);
    auto relin_keys = keygen.relin_keys(8);

    Plaintext plain1("1");
    Plaintext plain0("0");
    Plaintext plain;
    //Ciphertext encrypted1;
    //Ciphertext encrypted0;
    //cout << "Encrypting " << plain1.to_string() << ": ";
    //encryptor.encrypt(plain1, encrypted1);
    //cout << "Done" << endl;
    //cout << "Encrypting " << plain0.to_string() << ": ";
    //encryptor.encrypt(plain0, encrypted0);
    //cout << "Done" << endl;
    //cout << "Noise Budget: " << decryptor.invariant_noise_budget(encrypted1) 
//	    << " bits" << endl;
    
    Ciphertext list1[LISTLEN];
    Ciphertext list2[LISTLEN];
    Ciphertext listo[LISTLEN][LISTLEN];
    Ciphertext tmp;
    Plaintext D[LISTLEN][LISTLEN];
    cout << "Creating Lists:\t";
    auto t1=Clock::now();
    for(int i=0;i<LISTLEN;i++)
    {
...
//    evaluator.sub_inplace(list1[0],list1[0]);
//    fout.open("ZeroCipher.txt",fstream::out);
//    list1[0].save(fout);
//    fout.close();
    fout.open("myoutput.txt",fstream::out);
    decryptor.decrypt(list1[0],plain);	
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
			...
			r++;
		    sprintf(buffer,"%x",r);
		    D[i][j]=Plaintext(buffer);
	    }
    }  
    auto duration=timediff(t1,t2);
    cout << "Done" << endl << "It took: " << duration << "ns" <<endl;
    cout << "Noise Budget: " << decryptor.invariant_noise_budget(list1[0]) 
	    << " bits" << endl;
    cout << "Taking product of lists:\t" << flush;
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
	       fout << "Noise Budget before mult:" << 
		       decryptor.invariant_noise_budget(list1[i]) <<endl;
               t1=Clock::now();
	       evaluator.multiply(list1[i],list2[j],listo[i][j]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "Cyphertext mult took: " << duration << "ns" <<endl;
	       fout << "Noise Budget after mult:" << 
		       decryptor.invariant_noise_budget(listo[i][j]) <<endl;
               t1=Clock::now();
    	       evaluator.relinearize_inplace(listo[i][j], relin_keys);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "Relinearization took: " << duration << "ns" <<endl;
	       fout << "Noise Budget after relinear:" << 
		       decryptor.invariant_noise_budget(listo[i][j]) <<endl;
               //evaluator.add_inplace(listo[i+j],tmp);
	       //evaluator.multiply_plain_inplace(listo[i][j],D[i][j]);
	    }
    }
    cout << "Done" << endl;
    cout << "Taking product with matrix:\t" << flush;
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
	       //evaluator.multiply(list1[i],list2[j],listo[i][j]);
    	       //evaluator.relinearize_inplace(listo[i][j], relin_keys);
               //evaluator.add_inplace(listo[i+j],tmp);
	       fout << "Noise Budget before plainmult:" << 
		       decryptor.invariant_noise_budget(listo[i][j]) <<endl;
               t1=Clock::now();
	       cout << D[i][j].to_string() <<endl;
	       evaluator.multiply_plain_inplace(listo[i][j],D[i][j]);
    	       t2=Clock::now();
    	       duration=timediff(t1,t2);
    	       fout << "Plaintext mult took: " << duration << "ns" <<endl;
	       fout << "Noise Budget after plainmult:" << 
		       decryptor.invariant_noise_budget(listo[i][j]) <<endl;
	    }
    }
    cout << "Done" << endl;
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    for(int j=0;j<LISTLEN;j++)
	    {
		    ...
		    {
                            t1=Clock::now();
			    evaluator.add_inplace(tmp,listo[i][j]);
    	                    t2=Clock::now();
    	                    duration=timediff(t1,t2);
    	                    fout << "Summation took: " << 
				    duration << "ns" <<endl;
	       fout << "Noise Budget after sum:" << 
		       decryptor.invariant_noise_budget(tmp) <<endl;
		    }
	    }
    }
    ...
    //Plaintext plist[LISTLEN][LISTLEN];
    cout << "Decrypting:\t";
    /*for(int i=0;i<LISTLEN-1;i++)
    {
	for(int j=0;j<LISTLEN;j++)
	decryptor.decrypt(listo[i][j],plist[i][j]);	
    }*/
    decryptor.decrypt(tmp,plain);	

    cout << "Done" << endl;
    cout << "Noise Budget: " << decryptor.invariant_noise_budget(tmp) 
	    << " bits" << endl;
    ...
    for(int i=0;i<LISTLEN;i++)
    {
	    ...
	    for(int j=0;j<LISTLEN-1;j++)
	         cout << plist[i][j].to_string() << "|";
	    cout << plist[i][LISTLEN-1].to_string() <<"]" << endl;
    }
    cout << endl;*/
    cout << "The resulatant is: " << plain.to_string() << endl;
    fout.close();
}
void example_bfv_basics_i()
{
    ...
    */
    ...
    through the functions 
        ...
        DefaultParams::coeff_modulus_128bit(int)
    ...
    example DefaultParams::coeff_modulus_128bit(int) returns a vector of SmallModulus instances. 
...
void example_bfv_basics_ii()
{
    ...
    */
    ...
    be any integer at least 1 [dbc_min()] and at most 60 [DefaultParams::dbc_max()]. A large
    ...
    */
    auto relin_keys16 = keygen.relin_keys(16,8);
    ...
    evaluator.square_inplace(encrypted);
    cout << "Size after forth squaring: " << encrypted.size() << endl;
    cout << "Noise budget after forth squaring: "
        << decryptor.invariant_noise_budget(encrypted) << " bits" << endl;
...
void example_ckks_basics_iii()
{
    ...
    */
    ...
    We use the DefaultParams::small_mods_40bit(int) function to get primes from a hard-coded 

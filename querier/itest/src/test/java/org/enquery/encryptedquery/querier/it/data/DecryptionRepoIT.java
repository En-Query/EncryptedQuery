...
public class DecryptionRepoIT extends AbstractQuerierItest {
	...
	public void basicOperations() throws Exception {
		...
		Decryption decryption = sampleData.addDecryption(retrieval);
		decryption = decryptionRepo.add(decryption);

		InputStream inputStream = new ByteArrayInputStream("test-result-decryption".getBytes());
		decryptionRepo.updatePayload(decryption, inputStream);

		try (InputStream payloadInputStream = decryptionRepo.payloadInputStream(decryption)) {
			assertEquals("test-result-decryption", new String(IOUtils.toByteArray(payloadInputStream)));
		}

		Collection<Decryption> list = decryptionRepo.listForRetrieval(retrieval);
		assertEquals(1, list.size());

		Decryption d = list.iterator().next();
		assertEquals(decryption.getId(), d.getId());

		decryptionRepo.deleteAll();
		list = decryptionRepo.listForRetrieval(retrieval);

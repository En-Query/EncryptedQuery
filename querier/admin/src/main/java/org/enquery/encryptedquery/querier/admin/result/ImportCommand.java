 ...
 */
package org.enquery.encryptedquery.querier.admin.result;
 ...
 */
@Service
@Command(scope = "result", name = "import", description = "Import results.")
public class ImportCommand extends BaseCommand implements Action {
	...
	@Option(name = "--input-file",
			...
			multiValued = false,
			description = "Input results file to import.")
	...
	ResultUpdater getUpdater() {
		return updater;
	}

	void setUpdater(ResultUpdater updater) {
	...
	public Object execute() throws Exception {
		...
		try (InputStream is = Files.newInputStream(file)) {
			count = updater.importFromResultExportXMLStream(is);
		}

		printSuccess(String.format("%d results imported.", count));

...
public class MySQLDAOFactory extends DAOFactory {
	...
	public MySQLDAOFactory (Properties props) {
   
		logger.info("Inializing DAO Factory with Max Queue Size {} / Pause length between queue size checks ( {} ) in seconds, and max hits per selector {}",
				numFormat.format(maxQueueSize), pauseTimeForQueueCheck, numFormat.format(maxHitsPerSelector));
	   // Set the JDBC Driver
	   try {
           Class.forName("com.mysql.cj.jdbc.Driver").newInstance();
       } catch (Exception ex) {
          logger.error("Exception loading MySql Driver {}", ex.getMessage());
       }
...
//		logger.info("Connecting to MySql {}/{} database using username {} with SSL {}", dbUrl, dbName, dbUser, useSSL);
		try {
			String connectionUrl = "jdbc:mysql://" + dbUrl + "/" + dbName + "?autoReconnect=true&useSSL=" + useSSL.toString() + "&serverTimezone=UTC";
	...
	public long loadResponderDataDAO(QueryInfo queryInfo, String queryStatement, int hashGroupSize, 
		...
		try {
			stmt = dbConnection.createStatement(java.sql.ResultSet.TYPE_FORWARD_ONLY,

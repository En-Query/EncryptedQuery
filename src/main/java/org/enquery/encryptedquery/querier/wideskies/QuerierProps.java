...
public class QuerierProps {
	...
	static final String ENCQUERYMETHOD = "pir.encryptQueryMethod";
	...
	static void setEncryptionDefaults(Properties properties) {
		...
		if (!properties.containsKey(ENCQUERYMETHOD)) {
			properties.setProperty(ENCQUERYMETHOD, EncryptQuery.DEFAULT);
		}
	}
	...
	public static boolean validateQuerierEncryptionProperties(Properties properties) {
		...
		if (properties.containsKey(ENCQUERYMETHOD)) {
			if (!Arrays.asList(EncryptQuery.METHODS).contains(properties.getProperty(ENCQUERYMETHOD))) {
				valid = false;
			}
		}

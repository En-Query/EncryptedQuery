...
public class PrimeGenerator
{
  ...
  public static BigInteger getSinglePrime(int bitLength, int certainty, Random rnd)
  {
    ...
    while (true)
    {
      ...
      {
        // FIPS 186-4, step 4.5
        if (passesMillerRabin(p, numRounds, rnd))
        {
          // We have winner
          break;
        }
  ...
  public static BigInteger getSecondPrime(int bitLength, int certainty, Random rnd, BigInteger p)
  {
    ...
    while (true)
    {
      ...
      {
        // FIPS 186-4, step 5.6
        if (passesMillerRabin(q, numRounds, rnd))
        {
          // We have winner
          break;
        }
  ...
  /**
   * Method to return the number of Miller-Rabin rounds that we need
   * to perform when generating probable primes in order to be FIPS
   * 186-4 compliant in the case of bitLength = 1024, 2048, or 3072.
   * The values are from FIPS 186-4 Table C.2.  For other prime sizes
   * we try to return a reasonable value.
   * <p>
   * We ignore any Miller-Rabin rounds that the Java BigInteger
   * library may have already done.
   */
  private static int calcNumMillerRabinRounds(int bitLength)
  {
    int numRounds;
   ...
   * <p>
   * Follows Algorithm B.3.6 from FIPS 186-4.  Will optionally ensure that the prime meets the requirements in NIST SP 800-56B and FIPS 186-4
   * <p>
   * Returns an array containing the final prime p, followed by the first auxiliary prime p1 which divides p-1
   * <p>
   * NOTE: bitLength corresponds to the FIPS 186-4 nlen parameter
   * <p>
   * FIPS 186-4 only applies for bitLength=1024, 2048, and 3072.  For any other value of bitLength, use this method with caution.
   *
   * @throws IllegalArgumentException
   *           If {@code bitLength} is less than 1024
   * @throws PIRException
   *           If an unexpected error occured during prime generation
   */
  public static BigInteger[] getPrimePairWithAuxiliaryPrimes(int bitLength, int certainty, Random rnd) throws PIRException
  {
    logger.debug("bitLength " + bitLength + " certainty " + certainty + " random " + rnd);
    ...
    while (true)
    {
      ...
      if (passesMillerRabin(Xp1, rounds, rnd))
      {
        break;
      }
    }
    ...
    while (true)
    {
      ...
      int rounds = calcNumMRRoundsForAuxiliaryPrime(bitLength);
      if (passesMillerRabin(Xp2, rounds, rnd))
      {
        break;
      }
    }
    BigInteger p2 = Xp2;
    // step 4.3
    BigInteger[] pResult = getSinglePrimeFromAuxiliaryPrimes(p1, p2, bitLength, certainty, rnd);
    ...
    while (true)
    {
      ...
      while (true)
      {
        ...
        int rounds = calcNumMRRoundsForAuxiliaryPrime(bitLength);
        if (passesMillerRabin(Xq1, rounds, rnd))
        {
          break;
        }
      }
      ...
      while (true)
      {
        ...
        int rounds = calcNumMRRoundsForAuxiliaryPrime(bitLength);
        if (passesMillerRabin(Xq2, rounds, rnd))
        {
          break;
        }
      }
      BigInteger q2 = Xq2;
      // step 5.3
      BigInteger[] qResult = getSinglePrimeFromAuxiliaryPrimes(q1, q2, bitLength, certainty, rnd);
      q = qResult[0];
      BigInteger Xq = qResult[1];
      // step 6
      // Compute the minimumDifference 2^((bitLength/2) – 100) for use in FIPS 186-4 B.3.6, step 6
      BigInteger minimumDifference;
      if (!minimumDifferenceCache.containsKey(bitLength))
      {
        minimumDifference = BigDecimal.valueOf(2).pow(bitLength / 2 - 100).toBigInteger();
  ...
  /**
   * Method to generate a single probable prime from two auxiliary primes, following algorithm C.9 from FIPS 186-4.
   * <p>
   * Will optionally ensures that the prime meets the requirements from FIPS 186-4.
   * <p>
   * FIPS 186-4 only applies for bitLength=1024, 2048, and 3072.  For any other value of bitLength, use this method with caution.
   */
  public static BigInteger[] getSinglePrimeFromAuxiliaryPrimes(BigInteger r1, BigInteger r2, int bitLength, int certainty, Random rnd) throws PIRException
  {
    // step 1
    ...
    while (true)
    {
      ...
      while (true)
      {
        ...
        if (Y.isProbablePrime(certainty))
        {
          ...
          if (passesMillerRabin(Y, numRounds, rnd))
          {
            // we have a winner
  ...
  /**
   * Method to return the number of Miller-Rabin rounds that we need for auxiliary primes in addition to BigInteger's own primality testing
   */
  private static int calcNumMRRoundsForAuxiliaryPrime(int bitLength)
  {
  ...
  /**
   * Returns suggested lengths for the two auxiliary primes.  When
   * bitLength = 1024, 2048, or 3072, the values are based on FIPS
   * 186-4 Table B.1.  For other values > 1024, we try to return
   * reasonable values.
   *
   * @throws IllegalArgumentException
   *           If {@code bitLength} is smaller than 1024
   */
  private static int[] calcSuggestedAuxiliaryPrimeLengths(int bitLength)
  {
    int len1, len2;
